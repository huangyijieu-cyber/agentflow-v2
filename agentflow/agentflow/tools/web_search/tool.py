...
class Web_Search_Tool(BaseTool):
    ...
    def _get_website_content(self, url):
        ...
        response = self.gateway.fetch(url)
        text = response["text"][:self.max_window_size]
        if not text.strip():
            raise SearchGatewayError(
                "Search gateway returned no readable text",
                code="empty_fetch_response",
            )
        return text
    ...
    def execute(self, query, url):
        ...
        if not isinstance(url, str) or not url.strip():
            return "Error: Web search URL must be a non-empty string."
        ...
        try:
            ...
            # Deliberately do not fall back to direct public internet access.
            return f"Error: Web fetch via Search Gateway failed: {e}"
