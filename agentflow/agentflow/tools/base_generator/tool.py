...
class Base_Generator_Tool(BaseTool):
    ...
    def execute(self, query, image=None):
        ...
        try:
            input_data = [query]
            response = self.llm_engine(input_data[0], temperature=self.temperature, usage_by="[base generator] execute")
            return response
...
if __name__ == "__main__":
    ...
    # tool = Base_Generator_Tool(model_string="together-Qwen/Qwen2.5-7B-Instruct")

    tool = Base_Generator_Tool(model_string="vllm-Qwen3-30B-A3B-Instruct-2507")  ## vllm-{model_name}
