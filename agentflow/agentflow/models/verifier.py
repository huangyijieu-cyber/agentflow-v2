...
"""
        ...
        stop_verification = self.llm_engine_fixed(input_data[0], response_format=MemoryVerification, temperature=self.temperature, usage_by="[verifier] verificate context")
        if json_data is not None:
            json_data[f"verifier_{step_count}_prompt"] = input_data[0]
    ...
    def extract_conclusion(self, response: Any) -> Tuple[str, str]:
        if isinstance(response, str):
            ...
            try:
                ...
                print(f"Failed to parse response as JSON in Verifier: {str(e)}")
