...
def validate_local_model(model_string: str):
    return any(x in model_string.lower() for x in ["qwen"])


def validate_local_url(local_url: str):
...
class ChatOpenAI(EngineLM, CachedEngine):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.is_local_model = validate_local_model(self.model_string) or validate_local_url(kwargs.get("base_url", "None"))
    ...
    def _generate_text(
        ...
        ## otherwise local model
            ...
            print(f"Using local model: {self.model_string}")
            response = self.client.chat.completions.create(
                model=self.model_string,
                messages=[
                    {"role": "system", "content": sys_prompt_arg},
            ...
            #     # ✅ 关键修复：检查 choices 是否为空
            #     if not chunk.choices:
            #         # 这是最后一个 usage-only 的 chunk，可以跳过或提取 usage
            #         if chunk.usage:
            #             print(f"Usage: {chunk.usage}")
            #         continue  # 跳过空 choices
            ...
            #     # 可选：检测到 finish_reason='stop' 时提前退出
            #     if chunk.choices[0].finish_reason == 'stop':
            #         break
            # response = full_response

        else:
            raise ValueError(f"Not found model: {self.model_string}")
